...
int main(int argc, char **argv) {
  ...
  auto tree = factory.createTreeFromFi le(treePath);
  ...
  int timeRestriction = (1 + (generationCount % 2))*setTime;
  ...
  while (robot->step(timeStep) != -1) {
    ...
    if ((position[0] < 0.05 && position[0] > 0) && (position[1] < 0.35 && position[1] > 0.25) && (position[2] < 0.05 && position[2] > 0)) {
      ...
      if (!finishedObjectives) {
        ...
        // instead of using a randomly generated number, we will just use the generationCount. To the robot it makes no difference
    ...
    if (robotThinksFinished && robot->getTime() < timeRestriction && finishedObjectivesAgain) {
      timeBonus = true;
    }
